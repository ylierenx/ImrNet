...
# sample_and_inirecon_2 = model.sample_and_inirecon_2(num_filters=102, B_size=32)
...
if flag:
    dict1 = torch.load('./check_point1/spl_optical_flow_video_step1_0.5_0.1_5.ckpt')
...
# print(sample_and_inirecon_2.sample_transpose.weight)
# sys.exit()
params_ininet_2 = list(sample_and_inirecon_2.sample_transpose.parameters())
params_Biv_Shr_2 = list(Biv_Shr1_2.parameters()) + list(Biv_Shr2_2.parameters()) + list(Biv_Shr3_2.parameters())
params_wienet_2 = list(wiener_net1_2.parameters()) + list(wiener_net2_2.parameters()) + list(wiener_net3_2.parameters())
params_splnet_2 = params_ininet_2 + params_Biv_Shr_2 + params_wienet_2
optimizer_splnet_2 = optim.Adam(params_splnet_2, lr=0.0001)
...
if __name__ == '__main__':
    ...
    for epoch in range(-1 + 1, 3601):
        ...
        for i, (inputs, f_index) in enumerate(train_loader):
            ...
            all_loss = recnLoss_1_2 + 0.01 * (constrant_loss1 + constrant_loss2 + constrant_loss3)
            all_loss.backward()
            optimizer_splnet_2.step()

            if ((i % 100) == 0):
                print(f_index + 1)
                print(
                    " L0: %0.6f  L_i_pre: %0.6f L_i_forward: %0.6f L_i_backward: %0.6f  L8: %0.6f L_fusion: %0.6f L_new: %0.6f Iterations: %4d/%4d  " % (
                        recnLoss_0.item(), recnLoss_1_pre.item(), recnLoss_1_forward_1.item(),
                        recnLoss_1_backward_1.item(),
                        recnLoss_8.item(), recnLoss_1_1.item(), recnLoss_1_2.item(), i, len(train_loader)))
                f = open('train_0.5_0.1_step2.txt', 'a')
        ...
        if ((epoch % 50) == 0 and epoch > 0):
            dict1 = {
                ...
                'state_dict_optimizer_splnet_2': optimizer_splnet_2.state_dict(),
            ...
            torch.save(dict1, './check_point1' + "/spl_optical_flow_video_step2_0.5_0.1_" + str(checkpoint_counter) + ".ckpt")
            checkpoint_counter += 1
